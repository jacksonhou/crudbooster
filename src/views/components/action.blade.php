...
@if($button_action_style == 'button_text')
    ...
    @if(CRUDBooster::isUpdate() && $button_edit)
        <?php
        $query = $button_edit_show_condition;
        foreach ($row as $key => $val) {
            $query = str_replace("[" . $key . "]", '"' . $val . '"', $query);
        }
        ...
        @eval("if($query) {\$is_show = true;}else{\$is_show = false;}");
        ?>
        @if($is_show)
    ...
    @if(CRUDBooster::isDelete() && $button_delete)
        <?php
        $url = CRUDBooster::mainpath("delete/" . $row->$pk);
        $query = $button_delete_show_condition;
        foreach ($row as $key => $val) {
            $query = str_replace("[" . $key . "]", '"' . $val . '"', $query);
        }
        $query = $query == '' ? '1==1' : $query;
        ...
        ?>
        @if($is_show)
            <a class='btn btn-xs btn-warning btn-delete' title='{{trans("crudbooster.action_delete_data")}}'
               href='javascript:;'
...
@elseif($button_action_style == 'button_icon_text')
    ...
    @if(CRUDBooster::isDelete() && $button_delete)
        ...
        @if($is_show)
            <a class='btn btn-xs btn-warning btn-delete' title='{{trans("crudbooster.action_delete_data")}}'
               href='javascript:;'
               onclick='{{CRUDBooster::deleteConfirm($url)}}'><i
                        class='fa fa-trash'></i> {{trans("crudbooster.action_delete_data")}}</a>
        @endif
    @endif
            ...
            @if(CRUDBooster::isDelete() && $button_delete)
                ...
                @if($is_show)
                    <li><a class='btn-delete' title='{{trans("crudbooster.action_delete_data")}}' href='javascript:;'
                           onclick='{{CRUDBooster::deleteConfirm($url)}}'><i
                                    class='fa fa-trash'></i> {{trans("crudbooster.action_delete_data")}}</a></li>
                @endif
            @endif
        </ul>
    </div>
